...
selGenes <- dplyr::filter(gene.data, !hgnc_symbol %in% knownGenes$hgnc_symbol)
```
...
```{r pubmed}
selGenes <- dplyr::filter(selGenes, hgnc_symbol != "")
queryString <- paste(unique(selGenes$hgnc_symbol), "AND (heat shock OR stress)")

query <- lapply(queryString, EUtilsSummary)
names(query) <- unique(selGenes$hgnc_symbol)
...
putativeGenes <- dplyr::filter(limmaTable, SymbolReannotated %in% names(queryEmpty))
putativeGenes <- dplyr::left_join(putativeGenes, selGenes, by=c(SymbolReannotated="hgnc_symbol"))
```

```{r geneRecords, cache=FALSE}
write.table(data.frame(gene=names(queryFound), pmid=sapply(queryFound, paste, collapse=",")),
