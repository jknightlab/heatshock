...
snpRange <- unlist(lapply(geneSNPs, function(x) x$snp))
```
...
fam$distance[is.na(fam$distance)] <- -9
fam$direction[is.na(fam$direction)] <- -9
...
plink --bfile tmp/hapmap_yri.smpl_flt2 --out tmp/hapmap_yri.cis --prune --extract tmp/cisRanges --recode oxford gen-gz > log/extract_cis_geno.log
```
