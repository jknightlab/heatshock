...
plink --bfile tmp/hapmap_yri.smpl_flt2 --out tmp/hapmap_yri.cis --extract tmp/cisRanges --make-bed > log/extract_cis_geno.log
```
...
globalPheno <- data.frame(sample=stimComp$sample,
                          pheno1=sqrt((stimComp$t1 - basalComp$t1)^2 + (stimComp$t2 - basalComp$t2)^2),
                          pheno2=atan((stimComp$t2 - basalComp$t2)/(stimComp$t1 - basalComp$t1)),
                          stringsAsFactors = FALSE)

mvPheno <- geno$fam[,1:2]
names(mvPheno) <- c("FID", "IID")
mvPheno <- dplyr::left_join(mvPheno, globalPheno, by=c(IID="sample"))
write.table(mvPheno, file="tmp/hapmap_yri.cis.phen", row.names=FALSE, col.names=TRUE, 
            quote=FALSE, sep=" ")
```

```{r runMVassoc, engine="bash"}
plink.multivariate --bfile tmp/hapmap_yri.cis --out tmp/hapmap_yri.cis --mult-pheno tmp/hapmap_yri.cis.phen --mqfam --perm --missing-phenotype NA >  log/mult_pheno.log
```
