...
## Functions to select interesting genes for topGO analysis.
## Requires `tCut` to be defined in global environment.
selGenesUp <- function(score){
	score >= tCut
}

selGenesDown <- function(score){
	score <= -tCut
}

selGenesAny <- function(score){
	abs(score) >= tCut
}
...
```{r preTopGO}
tCut <- min(abs(limmaTable$t[limmaTable$adj.P.Val < 0.01]))
universe <- limmaTable$t
names(universe) <- limmaTable$EntrezReannotated
universe <- universe[!is.na(names(universe))]
tgUp <- new("topGOdata", ontology="BP", allGenes=universe, 
			geneSelectionFun=selGenesUp, nodeSize=10,
			description="Genes up-regulated after heat shock",
			annotationFun=annFUN.org, mapping="org.Hs.eg.db")
tgDown <- new("topGOdata", ontology="BP", allGenes=universe, 
			geneSelectionFun=selGenesDown, nodeSize=10,
			description="Genes down-regulated after heat shock",
			annotationFun=annFUN.org, mapping="org.Hs.eg.db")
tgAny <- new("topGOdata", ontology="BP", allGenes=universe, 
			geneSelectionFun=selGenesAny, nodeSize=10,
			description="Genes differentially expressed in response to heat shock",
			annotationFun=annFUN.org, mapping="org.Hs.eg.db")
```

```{r testTopGO}
resultUp <- runTest(tgUp, algorithm="elim", statistic="fisher")
resultDown <- runTest(tgDown, algorithm="elim", statistic="fisher")
```
...
as differentially expressed at an FDR threshold of 0.01 (see `r tabRef("topTable")`
...
limmaExtreme <- dplyr::filter(limmaTable, abs(logFC) > 0.5 & adj.P.Val < 0.01)
...
ggplot(limmaTable, aes(x=logFC, y=-log10(adj.P.Val))) + 
        ...
        geom_text(data=dplyr::filter(limmaExtreme, distance > (2*fcScale)^2 & logFC >= 1), 
        ...
        theme_bw() + xlab("log2(fold change)") + ylab("-log10(FDR)") + expand_limits(x=max(limmaTable$logFC)+0.25, 
            y=max(-log10(limmaTable$adj.P.Val))+2)
```

## GO enrichment analysis
The GO analysis highlights several categories that are significantly enriched amongst
...
it appears that genes upregulated in response to heat shock are predominantly
related to stress response (including *regulation of cellular response to heat* [GO:1900034]) 
...
include *positive regulation of B cell activation* [GO:0050871] as well as 
*regulation of B cell differentiation* [GO:0045577]. Other down regulated genes 
are annotated with *rRNA processing* [GO:0006364] and *ribosome biogenesis* [GO:0042254],
indicating a general reduction in translational activity.
...
```{r goPlot, cache=FALSE, fig.cap=figRef("goPlot", "GO category enrichment for differentially expressed genes.")}
...
fullPlot <- ggplot(tbl, aes(x=Expected, y=Significant)) + 
    ...
    theme_bw()
multiplot(fullPlot, fullPlot  + xlim(0,50) + ylim(0,100), cols=2)
```
