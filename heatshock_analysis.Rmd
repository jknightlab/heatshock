...
symbols <- data.frame(entrez=names(symbols), symbol=unlist(symbols))
...
group_up <- subset(limmaTable, limmaTable$adj.P.Val < fdrCut & limmaTable$logFC > fcCut)
group_dn <- subset(limmaTable, limmaTable$adj.P.Val < fdrCut & limmaTable$logFC < -fcCut)
group_others <- subset(limmaTable, limmaTable$adj.P.Val >= fdrCut | (limmaTable$logFC < fcCut & limmaTable$logFC > -fcCut) )
...
in the literature. This leaves `r length(queryEmpty)` genes with no obvious association to heat shock. Of these 
