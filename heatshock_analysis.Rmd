...
clusterOp2<- dplyr::filter(sig.pc, `Operator initial cRNA` == "PE" & !outlier)
...
clusterOp1 <- dplyr::filter(sig.pc, `Operator initial cRNA` == "CE" & 
		Sentrix_ID != 536403752 & !outlier)
...
dendroData$segments <- mutate(dendroData$segments, 
		cluster=ifelse(Sentrix_ID == 536403752, "1",
		ifelse(`Operator initial cRNA` == "PE", "2","3")))
