...
clustering by BeadChip, as well as two obvious outliers. Both samples for the 
individuals corresponding to these outliers were removed.
...
exclude$sample <- union(exclude$sample, c("NA18508", "NA19201"))
```

```{r vsnPCAcluster}
clusterOp<- dplyr::filter(sig.pc, Sentrix_ID %in% c(526786720, 526786721) & !outlier)
clusterOpBound_1_2 <- chull(clusterOp$PC1, clusterOp$PC2)
clusterOpBound_2_3 <- chull(clusterOp$PC2, clusterOp$PC3)
clusterOpBound_1_3 <- chull(clusterOp$PC1, clusterOp$PC3) 
...
clusterChip2 <- dplyr::filter(sig.pc, Sentrix_ID %in% c(536403729, 536403730) & !outlier)
...
clusterChip3 <- dplyr::filter(sig.pc, Sentrix_ID %in% c(526786722, 526786736, 
				526786743, 526786749) & !outlier)
...
clusterChip4 <- dplyr::filter(sig.pc, Sentrix_ID == 536403760 & !outlier)
clusterChip4Bound_1_2 <- chull(clusterChip4$PC1, clusterChip4$PC2)
clusterChip4Bound_1_3 <- chull(clusterChip4$PC1, clusterChip4$PC3)
clusterChip4Bound_2_3 <- chull(clusterChip4$PC2, clusterChip4$PC3)
```
...
ggplot(sig.pc, aes(x=PC1, y=PC3)) +  
	 geom_polygon(data=clusterOp[clusterOpBound_1_3, ], fill="lightgrey", colour="grey") +
	 geom_polygon(data=clusterChip1[clusterChip1Bound_1_3, ], alpha=0.8, fill="yellow", colour="orange") +
	 geom_polygon(data=clusterChip2[clusterChip2Bound_1_3, ], alpha=0.8, fill="lightblue", colour="blue") +
	 geom_polygon(data=clusterChip3[clusterChip3Bound_1_3, ], alpha=0.8, fill="lightgreen", colour="green") +
	 geom_polygon(data=clusterChip4[clusterChip4Bound_1_3, ], alpha=0.8, fill="lightpink", colour="red") +
...
of samples by BeadChip is apparent (`r figRef("exprDendro")`). This clustering suggests 
a further outlier from the chip with Sentrix ID 536403760 (the red cluster), which was 
also removed. The dendrogram also suggests that the red and green clusters should be 
merged, producing a final grouping into four clusters (`r figRef("vsnPCA2")` and 
`r figRef("exprDendro2")`).
...
dendroData$segments <- merge(dendroData$segments, info[info$Treatment == "Basal",c("Cell Line", "Sentrix_ID")], by.x="label", by.y="Cell Line", all.x=TRUE)
dendroData$segments <- mutate(dendroData$segments, cluster=ifelse(Sentrix_ID %in% c(526786720, 526786721), "1", ifelse(Sentrix_ID == 536403752, "2", 
        ifelse(Sentrix_ID %in% c(536403729, 536403730), "3", ifelse(Sentrix_ID %in% c(526786722, 526786736, 
				526786743, 526786749), "4", "5")))))
...
ggplot(dendroData$segments) + geom_segment(aes(x=x, y=y, xend=xend, yend=yend, linetype=treatment, colour=cluster)) + 
        ...
        scale_colour_manual(values=c("1"="grey", "2"="orange", "3"="blue", "4"="green", "5"="red", "internal"="black")) +
        theme_dendro() + theme(axis.text.x = element_text(angle=90, hjust=1))
```

```{r clusterExclude}
exclude$sample <- union(exclude$sample, "NA19209")
```
```{r excludeOutliers2, ref.label="filterSample"}
```

```{r exclOutlGeno2, ref.label="sampleList", cache=FALSE}
```

```{r vsn3, ref.label="vsn"}
```

```{r vsnPCA2, ref.label="vsnPCA"}
```

```{r vsnPCAcluster2}
clusterChip4<- dplyr::filter(sig.pc, Sentrix_ID %in% c(526786720, 526786721))
clusterChip4Bound_1_3 <- chull(clusterChip4$PC1, clusterChip4$PC3) 

clusterChip1 <- dplyr::filter(sig.pc, Sentrix_ID == 536403752)
clusterChip1Bound_1_3 <- chull(clusterChip1$PC1, clusterChip1$PC3)

clusterChip2 <- dplyr::filter(sig.pc, Sentrix_ID %in% c(536403729, 536403730))
clusterChip2Bound_1_3 <- chull(clusterChip2$PC1, clusterChip2$PC3)

clusterChip3 <- dplyr::filter(sig.pc, Sentrix_ID %in% c(526786722, 526786736, 
				526786743, 526786749, 536403760))
clusterChip3Bound_1_3 <- chull(clusterChip3$PC1, clusterChip3$PC3)
```

```{r vsnPCAplot2, cache=FALSE, fig.cap=figRef("vsnPCA2", "PCA plot of normalised gene expression estimates by sample. Final groupings used for batch correction are highlighted.")}
ggplot(sig.pc, aes(x=PC1, y=PC3)) +  
	 geom_polygon(data=clusterChip1[clusterChip1Bound_1_3, ], alpha=0.8, fill="yellow", colour="orange") +
	 geom_polygon(data=clusterChip2[clusterChip2Bound_1_3, ], alpha=0.8, fill="lightblue", colour="blue") +
	 geom_polygon(data=clusterChip3[clusterChip3Bound_1_3, ], alpha=0.8, fill="lightgreen", colour="green") +
	 geom_polygon(data=clusterChip4[clusterChip4Bound_1_3, ], alpha=0.8, fill="lightgrey", colour="grey") +
     geom_point(aes( colour=as.character(Sentrix_ID)), size=3) + 
     theme_bw() + scale_colour_discrete("BeadChip")
```

```{r exprClust2, ref.label="exprClust"}
```

```{r clustPlot2, cache=FALSE, fig.cap=figRef("exprDendro", paste("Dendrogram of gene expression samples. Colours of leaves correspond to clusters in", figRef("vsnPCA2")))}
dendroData <- dendro_data(exprClust)
labels <- strsplit(as.character(dendroData$labels$label), "_")
dendroData$labels$label <- sapply(labels, "[[", 1)
dendroData$labels$treatment <- sapply(labels, "[[", 2)
...
dendroData$segments <- merge(dendroData$segments, info[info$Treatment == "Basal",c("Cell Line", "Sentrix_ID")], by.x="label", by.y="Cell Line", all.x=TRUE)
dendroData$segments <- mutate(dendroData$segments, cluster=ifelse(Sentrix_ID %in% c(526786720, 526786721), "1", ifelse(Sentrix_ID == 536403752, "2", 
        ifelse(Sentrix_ID %in% c(536403729, 536403730), "3", "4"))))
dendroData$segments$cluster[is.na(dendroData$segments$cluster)] <- "internal"
dendroData$segments <- dendroData$segments[order(dendroData$segments$xend), ] 
...
ggplot(dendroData$segments) + geom_segment(aes(x=x, y=y, xend=xend, yend=yend, linetype=treatment, colour=cluster)) + 
        scale_x_discrete(label=filter(dendroData$segments, label != "internal")$label) + 
        scale_linetype_manual(values=c(Basal="solid", Stim="dashed", internal="solid")) +
        scale_colour_manual(values=c("1"="grey", "2"="orange", "3"="blue", "4"="green", "internal"="black")) +
        theme_dendro() + theme(axis.text.x = element_text(angle=90, hjust=1))
```

The samples were then normalised separately for each group and observed batch effects
corrected with ComBat [@Johnson07;@SVA15], preserving differences due to heat shock
stimulation.

```{r vsnBatch}
batch <- filter(select(dendroData$segments, sample, cluster), cluster != "internal" & 
			sample %in% colnames(sig.data.norm))
batch$cluster <- as.integer(batch$cluster)
sig.raw <- acast(sig.data.raw, formula=probe ~ sample + treatment, 
		value.var="intensity")
sig.raw <- sig.raw[, as.character(dendroData$segments$sample)]
vsnFit <- vsn2(sig.raw, strata=batch$cluster)
sig.data.norm <- predict(vsnFit, newdata=sig.raw, useDataInFit = TRUE)
```

```{r vsnBatchFit, cache=FALSE, fig.cap=figRef("vsnBatchFit", "VSN fit to stratified dataset.")}
meanSdPlot(vsnFit)
```
