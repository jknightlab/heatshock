...
## group should be a factor with levels "up", "down" and "neutral"
selGenesUp <- function(group){
	group == "up"
}

selGenesDown <- function(group){
	group == "down"
}

selGenesAny <- function(group){
	group %in% c("up", "down")
}
...
fdrCut <- 0.01
fcCut <- log2(1.2)
group <- factor(ifelse(limmaTable$adj.P.Val > fdrCut | abs(limmaTable$logFC) < fcCut, "neutral",
		ifelse(limmaTable$logFC > 0, "up", "down")))
universe <- group
