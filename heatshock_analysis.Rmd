...
sig.select <- arrange(sig.select, desc(probeMean), desc(probeSD))
...
dendroData$segments <- left_join(dendroData$segments, 
		info[info$Treatment == "Basal",c("Cell Line", "Sentrix_ID", "Operator initial cRNA")], 
		by=c(label="Cell Line"))
dendroData$segments <- left_join(dendroData$segments, sig.pc[, c("Cell Line", "cluster")],
		by=c(label="Cell Line"))
