...
There are `r nrow(dplyr::filter(r_promoter, HSFpeak == 1, novel == 1))` up-regulated
genes without an established role in the heat shock response that show evidence of 
...
ggplot(dplyr::filter(r_gene, expr=="up"), 
       aes(x=factor(HSFpeak, levels=c("0", "1"), labels=c("absent", "present")), 
           y=-log10(pwm.p.value), 
           fill=factor(novel, levels=c("0", "1"), labels=c("established", "novel")))) +
