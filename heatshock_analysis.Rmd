...
All probes considered to be differentially expressed following heat shock 
(FDR < `fdrCut` and fold change > `r 2^fcCut`) were further investigated for
...
selectedGenes <- select(selectedGenes, ArrayAddress, NuID, GenomicLocation)
...
write.table(selectedFC, file="/analysis/tmp/selected_probes_fc.tab", sep="\t", quote=FALSE)
```
...
write.table(probePos, file="/analysis/tmp/selected_probes_pos.tab", sep="\t", quote=FALSE)
```

```{r snpPos}
snpPos <- read_tsv("tmp/hapmap_yri.smpl_flt2.bim", col_names=FALSE)
snpPos <- select(snpPos, 2, 1, 4)
names(snpPos) <- c("snp", "chrom", "pos")
```

```{r snpPosUpdate}
snpPos <- by(snpPos, snpPos$chrom, function(x){
  ...
  if(nrow(snps) < nrow(x)) warning("Dropped ", nrow(x) - nrow(snps), " from chromosome ", x$chrom[1])
  select(snps, snp, chrom, loc)
})
```

```{r snpPosComb}
snpPos <- do.call(rbind, snpPos)
snpPos$chrom <- paste0(snpPos$chrom)
snpPos <- snpPos[order(snpPos$chrom, snpPos$loc),]
```

```{r writeSNPpos, cache=FALSE}
write.table(snpPos, file="tmp/snp_loc_GRCh37.tab", quote=FALSE, sep="\t")
```
