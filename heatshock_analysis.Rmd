...
```{r prepGeno}
names(geno$bed) <- str_extract(names(geno$bed), "NA[[:digit:]]+")
geno$bed <- geno$bed[geno$bim$snp, colnames(selectedFC)]
flip <- rowSums(geno$bed) > ncol(geno$bed)
if(any(flip)){
...
write.table(geno$bed, file="genotypes_GRCh37.tab", quote=FALSE, sep="\t")
system("gzip -c tmp/genotypes_GRh37.tab > tmp/genotypes_GRCh37.tab.gz")
```
