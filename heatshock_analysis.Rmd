...
pander(GenTable(tg$up, up=resultUp, down=resultDown, topNodes=20, orderBy="up"))
```

```{r tableTopGODown, chache=FALSE}
set.caption(tabRef("topGODown", "Top 20 GO categories enriched for down regulated genes"))
pander(GenTable(tg$down, down=resultDown, up=resultUp, topNodes=20, orderBy="down"))
```
...
for genes differentially expressed between clusters 1 and 2. This reveals that genes with incresed
...
tab <- GenTable(tg_pls$up, cluster1=resultPlsUp, cluster2=resultPlsDown, topNodes=20, orderBy="cluster1")
tab[["cluster1 FDR"]] <- p.adjust(tab$cluster1)
tab[["cluster2 FDR"]] <- p.adjust(tab$cluster2)
tab <- dplyr::select(tab, -cluster1, -cluster2)
pander(tab)
```
...
tab <- GenTable(tg_pls$down, cluster2=resultPlsDown, cluster1=resultPlsUp, topNodes=20, orderBy="cluster2")
tab[["cluster1 FDR"]] <- p.adjust(tab$cluster1)
tab[["cluster2 FDR"]] <- p.adjust(tab$cluster2)
tab <- dplyr::select(tab, -cluster1, -cluster2)
pander(tab)
