...
for(i in 1:nPerm){
  ...
  assocPerm <- mPhen(cisGeno, as.matrix(permPheno), covariates="sex")
...
were randomly assigned to one of the observed set of genotypes.

```{r permFDR}
assoc$adj.p.val <- sapply(assoc$Results[, "JointModel", 2], function(x, p) sum(p < x)/length(p), pPerm)
```
...
```{r treatPLSpval, cache=FALSE, fig.cap=figRef("permPval", "Most extreme p-values observed after permutations.")}
ggplot(permResult, aes(x=-log10(p.value))) + 
  geom_histogram(fill="lightgrey", binwidth=0.1) +
  geom_vline(xintercept=-log10(assoc$Results[1,5,2]), colour="red") + 
