...
globalSNPs <- t(geno$bed[, selGlobal, drop=FALSE])
```

```{r writeGlobalSNPs, cache=FALSE}
writeTable(globalSNPs, file="tmp/global_geno.tab", sep="\t", quote=FALSE)
```
...
if(nPC_stim > 0) covarStim <- rbind(covarStim, t(pc_stim[, 1:nPC_stim]))
...
if(nPC_basal > 0) covarbasal <- rbind(covarbasal, t(pc_basal[, 1:nPC_basal]))
```
...
snpsGlobal$LoadFile("tmp/genotypes_GRCh37.tab")
...
meGlobalStim <- Matrix_eQTL_main(
    ...
    gene = phenoStim,
    cvrt = covarStim,
    output_file_name.cis = "tmp/eQTL_stim_global_analysis",
    pvOutputThreshold = 1,
    useModel = modelLINEAR, 
    errorCovariance = numeric(),
    verbose = FALSE,
...
meGlobalBasal <- Matrix_eQTL_main(
    ...
    gene = phenoBasal,
    cvrt = covarBasal,
    output_file_name.cis = "tmp/eQTL_basal_global_analysis",
    pvOutputThreshold = 1,
    useModel = modelLINEAR, 
    errorCovariance = numeric(),
    verbose = FALSE,
