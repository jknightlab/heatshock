...
for(i in 1:nPerm){
  ...
  tryCatch( assocPerm <- mPhen(permGeno, as.matrix(permPheno), covariates="sex"),
            error={
              permPheno <- dplyr::sample_n(as.data.frame(globalPheno), nrow(globalPheno), replace=FALSE)
              assocPerm <- mPhen(permGeno, as.matrix(permPheno), covariates="sex")
            })
...
```{r permFDR}
assoc$adj.p.val <- numeric(length(assoc$maf))
names(assoc$adj.p.val) <- dimnames(assoc$Results)[[1]]
for(i in 1:ncol(pPerm)){
  assoc$adj.p.val[binSNPs[[i]]] <- sapply(assoc$Results[binSNPs[[i]], "JointModel", 2],
...
```{r treatPLSpval, cache=FALSE, fig.cap=figRef("permPval", paste0("P-values observed after permutation of global response phenotype for SNP ", dimnames(assoc$Result)[[1]][1], "."))}
ggplot(data.frame(p.value=pPerm[,bins[dimnames(assoc$Result)[[1]][1]]]), aes(x=-log10(p.value))) + 
