...
system("tar -czf yri_geno.tar.gz -C hapmap_yri.smpl_flt2.bed hapmap_yri.smpl_flt2.bim hapmap_yri.smpl_flt2.fam")
```
...
system("tar -czf topGO.tar.gz -C tmp topGO_upregulated.tab topGO_downregulated.tab")
```
...
system("tar -czf reQTL_input.tar.gz -C tmp selected_probes_pos.tab snp_loc_GRCh37.tab selected_probes_fc.tab genotypes_GRCh37.tab covariates.tab")
```
...
system("tar -czf eQTL_input.tar.gz -C tmp selected_probes_pos.tab tmp/snp_loc_GRCh37.tab selected_probes_stim_expr.tab selected_probes_basal_expr.tab covar_expr_stim.tab covar_expr_basal.tab")
```
...
if(file.exists("heatshock_analysis_files/figure-html") && 
     !file.exists("heatshock_analysis_files/figure-html/figures_png.tar.gz")){
  system("(cd heatshock_analysis_files/figure-html && tar -czf figures_png.tar.gz *)")
}
if(file.exists("heatshock_analysis_files/figure-latex") && 
     !file.exists("heatshock_analysis_files/figures_pdf.tar.gz")){
  system("(cd heatshock_analysis_files/figure-latex && tar -czf figures_pdf.tar.gz *)")
}
```
