...
```{r plinkSNPs, engine="bash", cache.extra=list(file.info("data/hapmap_YRI_r23a_filtered.bed")$mtime, file.info("tmp/sample.lst")$mtime)}
plink --bfile data/hapmap_YRI_r23a_filtered --out tmp/hapmap_yri.snp_flt --keep tmp/sample.lst --maf 0.1 --geno 0.1 --hwe 1e-4 --autosome --make-bed --silent
```
...
`r totalSNPs` (`r format((totalSNPs - remainSNPs)/totalSNPs*100, nsmall=2, digits=2)`%);

```{r plinkSample, engine="bash", cache.extra=list(file.info("tmp/hapmap_yri.snp_flt.bed")$mtime)}
plink --bfile tmp/hapmap_yri.snp_flt --out tmp/hapmap_yri.smpl_flt --mind 0.1 --neighbour 1 3 --make-bed --silent
```
...
set.caption(tabRef("nnIBS", 
		"Pairs of Yoruban HapMap samples with evidence of increased genetic similarity (IBS-based nearest neighbour Z-score > 0)."))
pander(nnSuspect)
```
