...
globalPheno <- rbind(c("0", "P", "P"), globalPheno)

sample <- read.table("tmp/hapmap_yri.cis.sample", header = TRUE)
sample <- dplyr::select(sample, -phenotype)

sample <- dplyr::left_join(sample, globalPheno, by=c(ID_2="sample"))
sample$missing <- as.character(sample$missing)
write.table(sample, file="tmp/hapmap_yri.cis.sample", row.names=FALSE, col.names=TRUE, 
            quote=FALSE, sep=" ")
```

```{r runSNPTEST, engine="bash", cache=FALSE}
snptest -data tmp/hapmap_yri.cis.gen.gz tmp/yri_hapmap.cis.sample \
  -o tmp/hapmap_yri.cis
  -bayesian 1 \
  -method expected \
  -mpheno distance direction \
  -prior_qt_mean_b 0 \
  -prior_qt_V_b 0.02 \
  -prior_mqt_c 4 \
  -prior_mqt_Q 6
