---
title: Data analysis for *Characterisation of the global transcriptional response to heat shock and the 
impact of individual genetic variation*
author:
  - name: Peter Humburg
...
plsPlot <- function(plsComp, genotype, groups="treatment", type=c("present", "genotype")){
  type <- match.arg(type)
  treatGrp <- by(dplyr::select(plsComp, t1, t2),
                 plsComp[groups], colMeans)
  ...
  if(length(groups) > 1) treatAvg$sample <- treatAvg[[groups[2]]]
  else treatAvg$sample <- "avg"
  ...
  if(!missing(genotype)){
    ...
    grpMean <- by(dplyr::select(plsComp, t1, t2), c(plsComp[genotype],
                                                       plsComp[groups]), colMeans)
    ...
    if(length(groups) > 1){
      genoAvg$sample <- paste(genoAvg[[genotype]], genoAvg[[groups[2]]], sep="_")
    } else{
  ...
  if(length(groups) > 1){
    fig <- ggplot(plsComp, aes_string(x="t1", y="t2", group="sample", shape=groups[2]))
  } else {
    fig <- ggplot(plsComp, aes_string(x="t1", y="t2", group="sample"))
  }
  if(missing(genotype)){
    fig <- fig + geom_point(data=treatAvg, aes_string(colour=groups[1]), size=5)
  } else {
...
formatQTL <- function (me, limmaTable) {
  eQTL <- me$all$eqtls
  eQTL <- dplyr::left_join(eQTL, dplyr::select(limmaTable, NuID, SymbolReannotated, AveExpr,  logFC), by=c(gene="NuID"))
  eQTL <- dplyr::select(eQTL, snps, SymbolReannotated, gene, beta:FDR, AveExpr, logFC)
...
plsLimma <- function(data, cluster) {
  ...
  contrasts <- makeContrasts(cluster3-cluster1, cluster3-cluster2, cluster1-cluster2, levels=design)
  clusterFit <- lmFit(data, design)
  clusterFit2 <- contrasts.fit(clusterFit, contrasts)
  clusterFit2 <- eBayes(clusterFit2)

  clust3_1Table <- topTable(clusterFit2, coef=1, n=dim(clusterFit2)[1], adjust="BH")
  clust3_2Table <- topTable(clusterFit2, coef=2, n=dim(clusterFit2)[1],adjust="BH")
  clust1_2Table <- topTable(clusterFit2, coef=3, n=dim(clusterFit2)[1], adjust="BH")

  clust3_1Table <- cbind(ArrayAddress=rownames(clust3_1Table), clust3_1Table)
  clust3_1Table <- inner_join(dplyr::select(annot, ArrayAddress, NuID, SymbolReannotated, EntrezReannotated, GenomicLocation),
      clust3_1Table)
  ...
  clust3_2Table <- inner_join(dplyr::select(annot, ArrayAddress, NuID, SymbolReannotated, EntrezReannotated, GenomicLocation),
      clust3_2Table)
  ...
  clust1_2Table <- inner_join(dplyr::select(annot, ArrayAddress, NuID, SymbolReannotated, EntrezReannotated, GenomicLocation),
      clust1_2Table)
...
Gene N4BP2L2 is associated with two different probes that show evidence of differential expression. One appears to
be significantly up regulated while the other is down regulated following heat shock (`r tabRef('N4BP2L2')`). We note that this is the
only instance in which multiple probes for a differentially expressed gene provide conflicting evidence in this way.
While it is possible that these probes measure two different transcripts that exhibit opposing responses to heat shock
...
tab <- dplyr::select(tab, NuID,	SymbolReannotated,	logFC,	AveExpr,	t,	P.Value,	adj.P.Val,	B)
pander(tab)
```

```{r exludeInconsistent}
...
termFilter_extended <- c("death", "apoptosis", "apoptotic",
...
strictGenes <- dplyr::filter(gene.data, str_detect(name_1006, paste(termFilter_strict, collapse="|")), 
                             !str_detect(name_1006, paste(termFilter_viral, collapse="|")))
...
selGenes_strict <- dplyr::filter(limmaTable, adj.P.Val < fdrCut, logFC > fcCut,
                          !SymbolReannotated %in% knownList$strict,
...
gene.data_all <- getBM(
  attributes=c('hgnc_symbol', 'entrezgene', 'go_id', 'name_1006', 'namespace_1003'),
  filters = 'entrezgene', values = unique(limmaTable$EntrezReannotated), mart = mart)
filterTotal_strict <- dplyr::filter(gene.data_all, str_detect(name_1006, paste(termFilter_strict, collapse="|")), 
                             !str_detect(name_1006, paste(termFilter_viral, collapse="|")))
filterTotal_extended <- dplyr::filter(gene.data_all, str_detect(name_1006, paste(termFilter, collapse="|")),
                               !str_detect(name_1006, paste(termFilter_viral, collapse="|")))
```
...
for(i in 1:length(query)){
  ...
  query[[i]] <- tryCatch(EUtilsSummary(queryString[i]),
                           error=function(e) {
                             Sys.sleep(2)
...
for(i in 1:length(pmids)){
  if(i %% 10 == 0) Sys.sleep(1)
  pmids[[i]] <- tryCatch(PMID(EUtilsGet(queryFound[[i]])),
                           error=function(e) {
                             Sys.sleep(2)
...
putativeGenes_strict <- dplyr::filter(selGenes_strict, SymbolReannotated %in% names(queryEmpty))
...
for(g in unique(peaks2genes$cond2) ){
        ...
        hsfPval <- rbind(hsfPval, data.frame(cond=g, up=rst_up$p.value, down=rst_down$p.value, up.or=rst_up$estimate,
                                             down.or=rst_down$estimate,
...
r_promoter$novel <- ifelse(r_promoter$SymbolReannotated %in% knownList$novel, 3, 
                           ifelse(r_promoter$SymbolReannotated %in% knownList$extended & !r_promoter$SymbolReannotated %in% knownList$strict, 2,
                                  ifelse(r_promoter$SymbolReannotated %in% knownList$strict, 1,0)))
...
genotypes for SNPs close to differentially expressed genes and their upstream 
regulators (identified via Ingenuity Pathway Analysis) using the *MultiPhen* R
...
selectedGenes <- rbind(selectedGenes, dplyr::filter(limmaTable, SymbolReannotated %in% regs[["Upstream Regulator"]]))
...
Following up on the observed associations between SNPs and the global heat shock response we
investigated possible associations between these SNPs and differentially expreressed
genes implicated in the heat shock response. Here we define the gene set of interest
as those genes that were
significantly up regulated (FDR < `r fdrCut` and fold change > `r 2^fcCut`)
following heat shock and that were found to have HSF1 or HSF2 ChIP-seq peaks
or binding motifs (P < 0.05) in their promoter region.

```{r selectHeatShock}
selSymbol <- dplyr::filter(r_gene, expr == "up", HSFpeak == 1 | pwm.p.value < 0.05)$SymbolReannotated
...
probePos <- t(sapply(selectedGenes$GenomicLocation,
                     str_extract, c("^chr([[:digit:]]+|X)(_[[:alnum:]]+_hap[[:digit:]]+)*",
                                    ":[[:digit:]]+", "[[:digit:]]+:[+-]")))
...
write.table(covarFC, file="tmp/covariates.tab", quote=FALSE, sep="\t")
```
...
```{r prepGlobalQTL, results="hide", cache=FALSE}
...
```{r runGlobalMatrixEQTL, results="hide", cache=FALSE}
meGlobal <- Matrix_eQTL_main(
    snps = snpsGlobal,
    gene = pheno,
    cvrt = covar,
    output_file_name = "tmp/reQTL_global_analysis",
    pvOutputThreshold = 1,
...
```{r prepGlobalEQTL, results="hide", cache=FALSE}
...
```{r runGlobalEQTL, cache=FALSE, results="hide"}
meGlobalStim <- Matrix_eQTL_main(
    snps = snpsGlobal,
    gene = phenoStim,
    cvrt = covarStim,
    output_file_name = "tmp/eQTL_stim_global_analysis",
    pvOutputThreshold = 1,
    useModel = modelLINEAR,
    errorCovariance = numeric(),
    verbose = FALSE,
...
meGlobalBasal <- Matrix_eQTL_main(
    snps = snpsGlobal,
    gene = phenoBasal,
    cvrt = covarBasal,
    output_file_name = "tmp/eQTL_basal_global_analysis",
    pvOutputThreshold = 1,
    useModel = modelLINEAR,
    errorCovariance = numeric(),
    verbose = FALSE,
...
```{r globalEQTLout, include=FALSE, cache=FALSE}
tab_basal <- read.table('tmp/eQTL_basal_global_analysis', header=TRUE)
tab_basal <- dplyr::right_join(dplyr::select(limmaTable, NuID, SymbolReannotated), tab_basal, by=c(NuID='gene'))
tab_stim <- read.table('tmp/eQTL_stim_global_analysis', header=TRUE)
tab_stim <- dplyr::right_join(dplyr::select(limmaTable, NuID, SymbolReannotated), tab_stim, by=c(NuID='gene'))
write.table(tab_basal, file='tmp/eQTL_basal_global_analysis', sep='\t', quote=FALSE, row.names=FALSE)
write.table(tab_stim, file='tmp/eQTL_stim_global_analysis', sep='\t', quote=FALSE, row.names=FALSE)
rm(tab_basal, tab_stim)
system("tar -czf tmp/global_eqtl_analysis.tar.gz -C tmp eQTL_basal_global_analysis eQTL_stim_global_analysis")
```
### Interpreting the PLS components
Further considering the two PLS components it is apparent that the first
component has a strong correspondence with the heat shock response, i.e. it
separates basal from stimulated samples, whereas the interpretation of the
second component is less obvious. We note that treated samples exhibit
increased variance in the second component compared to basal samples. We note
that the second component does reveal some structure, presenting three distinct
clusters, within the stimulated samples but it is not immediately obvious what
is underlying this grouping. In an attempted ti elucidate the drivers behind this
structure we first assigned each stimulated sample to one of three clusters and
then considered patterns of differential expression between them.
...
```{r writeClusterDiffExpr, cache=FALSE}
write.table(clust1_2Table, file="/analysis/tmp/cluster_1_2_diff_expression.tab", row.names=FALSE, quote=FALSE, sep="\t")
```
...
```{r writeTopGO_pls, include=FALSE, cache=FALSE}
...
write.table(goTabPlsUp,
            file="tmp/topGO_pls_cluster_upregulated.tab", sep="\t", quote=FALSE, row.names=FALSE)
write.table(goTabPlsDown,
            file="tmp/topGO_pls_cluster_downregulated.tab", sep="\t", quote=FALSE, row.names=FALSE)
system("tar -czf tmp/topGO_pls_cluster.tar.gz -C tmp topGO_pls_cluster_upregulated.tab topGO_pls_cluster_downregulated.tab")
```


## Response QTL analysis

Genes associated with the heat shock response, either
through previous work or implicated by the analysis presented here, were
selected for further analysis as above to determine genetic factors that modulate
their response to the stimulus. 

For each of the `r nrow(selectedGenes)` probes the observed response to the
stimulus (as measured by the log~2~fold change) is tested for association with
the genotypes within a $`r cisWindow`$bp window either side of the probe location.
For this purpose only typed SNPs that passed QC were considered.

```{r writeProbePos, cache=FALSE}
write.table(probePos,
            file="tmp/selected_probes_pos.tab", sep="\t", quote=FALSE,
            row.names=FALSE, col.names=c("probe", "chrom", "start", "end"))
```
...
probePos <- read.table("tmp/selected_probes_pos.tab", header=TRUE)
```
...
geneP <- me$cis$min.pv.gene * sapply(names(me$cis$min.pv.gene),
                                     function(x) if(length(geneSNPs[[x]])) length(geneSNPs[[x]]) else 1)
...
geneP <- dplyr::arrange(geneP, Gene.FDR, SNP.pvalue)
geneP <- dplyr::filter(geneP, !duplicated(Symbol))
```
...
```{r bundleReQTL, cache=FALSE, include=FALSE}
...
system(paste("tar -czf tmp/reQTL_result.tar.gz -C tmp reQTL_analysis", paste(paste0("reQTL_", selGenes$Symbol,"analysis"), collapse=" ")))
```
...
geneP_stim <- dplyr::rename(geneP_stim, Symbol=SymbolReannotated, SNP=snps, SNP.pvalue=pvalue,
                       expression.FDR=adj.P.Val, SNP.beta=beta)
geneP_stim <- dplyr::arrange(geneP_stim, Gene.FDR, SNP.pvalue)
geneP_stim <- dplyr::filter(geneP_stim, !duplicated(Symbol))
...
geneP_basal <- dplyr::rename(geneP_basal, Symbol=SymbolReannotated, SNP=snps, SNP.pvalue=pvalue,
                       expression.FDR=adj.P.Val, SNP.beta=beta)
geneP_basal <- dplyr::arrange(geneP_basal, Gene.FDR, SNP.pvalue)
geneP_basal <- dplyr::filter(geneP_basal, !duplicated(Symbol))
```
...
for(i in 1:nrow(selGenes_stim)){
  ...
  meGene_stim[[selGenes_stim$NuID[i]]] <- Matrix_eQTL_main(
    ...
    gene = pheno,
    cvrt = covarStim,
    output_file_name = paste0("tmp/eQTL_", selGenes_stim$Symbol[i],"_stim_analysis"),
    pvOutputThreshold = 1,
...
geneSNPcount_stim <- sapply(meGene_stim, function(x) sum(x$all$eqtls$FDR < 0.01))
...
for(i in 1:nrow(selGenes_basal)){
  ...
  meGene_basal[[selGenes_basal$NuID[i]]] <- Matrix_eQTL_main(
    ...
    gene = pheno,
    cvrt = covarBasal,
    output_file_name = paste0("tmp/eQTL_", selGenes_basal$Symbol[i],"_basal_analysis"),
    pvOutputThreshold = 1,
    pvOutputThreshold.cis = 0,
    useModel = modelLINEAR,
    errorCovariance = numeric(),
...
geneSNPcount_basal <- sapply(meGene_basal, function(x) sum(x$all$eqtls$FDR < 0.01))

stimEQTL <- do.call(rbind, lapply(meGene_stim, formatQTL, limmaTable))
basalEQTL <- do.call(rbind, lapply(meGene_basal, formatQTL, limmaTable))
```
...
```{r bundleEQTL, cache=FALSE, include=FALSE}
...
system(paste("tar -czf tmp/eQTL_result.tar.gz -C tmp eQTL_basal_analysis eQTL_stim_analysis", paste(paste0("eQTL_", selGenes_basal$Symbol,"_basal_analysis"), collapse=" "), paste(paste0("eQTL_", selGenes_stim$Symbol,"_stim_analysis"), collapse=" ")))
```
...
sortedTable <- dplyr::arrange(dplyr::filter(limmaTable, adj.P.Val < fdrCut, abs(logFC) > fcCut), desc(logFC))
sideCols <- c(Basal="purple", Stim="orange")
treat <- str_extract(colnames(sig.data.norm), "(Basal|Stim)$")
heatmap.2(sig.data.norm[sortedTable$ArrayAddress, ], trace = "none", Rowv=FALSE, Colv=TRUE, scale="row", dendrogram="column", col=rev(brewer.pal(11, "RdBu")), ColSideColors=sideCols[treat], key=FALSE, labRow="", labCol="", xlab="Samples", ylab="Genes")
```

## GO enrichment analysis
The GO analysis highlights several categories that are significantly enriched among
the up and down regulated genes (`r sum(goTabUp[['up FDR']] < 0.05)` and
`r sum(goTabDown[['down FDR']] < 0.05)` categories with an FDR < 0.05 respectively).
...
*protein refolding* [GO:0042026]) whereas the leading GO categories for down regulated genes
...
and *chemokine production* [GO:0032602] but are not sufficiently enriched to be 
...
tbl <- bind_rows(gather(tblUp, "direction", "p.value", cluster1), gather(tblDown, "direction", "p.value", cluster2))
...
Of the `r length(unique(dplyr::filter(limmaTable, adj.P.Val < fdrCut & logFC > fcCut)$SymbolReannotated))` significantly up regulated genes
`r sum(goKnownList$known %in% dplyr::filter(limmaTable, adj.P.Val < fdrCut & logFC > fcCut)$SymbolReannotated)`
have been directly linked to the heat shock response and
`r length(goKnownList$strict)`
are associated with GO terms that clearly relate to heat shock response (Fisher's exact test P-value: `r pander(go.test_strict$p.value)`)
and a further `r length(knownList$strict) - length(goKnownList$strict)` are otherwise linked to the heat shock 
response in the literature. This leaves
`r length(knownList$novel) + length(setdiff(knownList$extended, knownList$strict))` genes with no obvious 
association to heat shock. Using a more inclusive definition of heat shock related processes, 
also including GO terms related to cell death and proliferation, accounts for an additional
`r length(setdiff(knownList$extended, knownList$strict))` genes, leaving `r length(knownList$novel)`
genes without any known heatshock association.
...
tab <- data.frame(Condition=condition,
                  up=paste0(hsfPval$sigGenes_up, " (P=", format(hsfPval$up, digits=2),", OR=", format(hsfPval$up.or, digits=2), ")"),
                  down=paste0(hsfPval$sigGenes_down, " (P=", format(hsfPval$down, digits=2),", OR=", format(hsfPval$down.or, digits=2),")"))
...
There are `r length(unique(dplyr::filter(r_promoter, HSFpeak == 1, novel == 3)$SymbolReannotated))` up-regulated
genes without any established role,
`r length(unique(dplyr::filter(r_promoter, HSFpeak == 1, novel == 2)$SymbolReannotated))` genes with limited
evidence and `r length(unique(dplyr::filter(r_promoter, HSFpeak == 1, novel == 1)$SymbolReannotated))` 
with substantial evidence
for their involvement in the heat shock response that show
...
ggplot(dplyr::filter(r_gene, expr=="up"),
       aes(x=factor(HSFpeak, levels=c("0", "1"), labels=c("absent", "present")),
           y=-log10(pwm.p.value),
           fill=factor(novel, levels=c("0", "1", "2", "3"), labels=c("established", "strong", "limited", "novel")))) +
...
```{r treatPLSplot, cache=FALSE, fig.width=10, fig.cap=figRef("treatPLS", "Samples separate clearly by treatment, showing a consistent global effect on gene expression from heat shock. Stimulated samples show evidence of three distinct groups.")}
...
```{r treatPLSpval, cache=FALSE, fig.cap=figRef("permPval", paste0("P-values observed after permutation of global response phenotype for SNPs ", dimnames(assoc$Result)[[1]][1], " (top) and ", dimnames(assoc$Result)[[1]][3], " (bottom). The distribution of p-values for each snp after permutation is shown on the left and the minimum p-value observed in each iteration across all SNPs on the right."))}
...
fig3 <- ggplot(data.frame(p.value=pPerm[,bins[dimnames(assoc$Result)[[1]][3]]]), aes(x=-log10(p.value))) +
  geom_histogram(fill="lightgrey", binwidth=0.1) +
...
`r figRef("plsGeno")` shows two examples of genotypes associating with the
global heat shock response. Note that the second one (`r dimnames(assoc$Results)[[1]][3]`)
is located within a CTCF binding site immediately downstream of CDKN1A,
an important regulator of cell cycle progression. This SNP is also found to be
associated with the expession level of HSF1 in resting lymphocytes (`r tabRef('globalSNPGenes_basal')`)
It may be interesting to note that
allele frequencies vary markedly across populations. While the frequency of the
ancestral T allele is 79% in the Yoruba 1000 genomes samples it is only observed with
a frequency of 24% in European samples.


```{r plsGenoPlot, cache=FALSE, fig.cap=figRef("plsGeno", paste0("Global response to heat shock by genotype for ", dimnames(assoc$Results)[[1]][1], " (left) and ", dimnames(assoc$Results)[[1]][3], " (right). Each individual is represented by two points corresponding to basal and stimulated state with arrows connecting paired samples. Genotypes are indicated by colour with blue corresponding to homozyguous carriers of the major allele and red indicating the presence of at least one minor allele."))}
...
```{r globalSNPGenes_fc, cache=FALSE}
tab <- meGlobal$all$eqtls
n <- max(sum(tab$FDR < 0.05), 5)
cut <- tab$FDR[n]
n <- min(max(which(tab$FDR <= cut)), 20)

tab <- dplyr::left_join(tab, dplyr::select(limmaTable, NuID, SymbolReannotated), by=c(gene="NuID"))
tab <- dplyr::rename(tab, Gene=SymbolReannotated)
tab <- dplyr::select(tab, SNP=snps, Gene, NuID=gene, Statistic=statistic, `p-value`=pvalue, FDR, beta)
...
```{r globalSNPGenes_fcPlot, cache=FALSE, fig.cap=figRef("globalGenesFC", "Heat shock response by genotype.")}
n <- sum(tab$FDR < 0.05)
if(n < 2) n <- 2
...
for(i in 1:n){
  ...
  fig[[i]] <- ggplot(plotData, aes(x=factor(genotype), y=fold_change)) +
    ...
    theme_bw() + xlab(snp) + ylab(paste(gene, "(log fold change)"))
}
cols <- 1
if(n > 1) cols <- 2
if(n > 4) cols <- 3
...
```{r globalSNPGenes_stim, cache=FALSE}
...
tab <- dplyr::select(tab, SNP=snps, Gene, Statistic=statistic, `p-value`=pvalue, FDR, beta)
...
```{r globalSNPGenes_basal, cache=FALSE}
...
```{r globalSNPHSF1_basal, cache=FALSE, fig.cap=figRef('globalSNPHSP1_basal', paste0('Expression of ', tab[1,'Gene'], ' in the basal state by genotype for ', tab[1, 'SNP'], '.'))}
#snp <- as.character(tab$SNP[1])
#gene <- as.character(tab$Gene[1])
#probe <- as.character(tab$NuID[1])

#fit <- lm(basalExpr[probe, ] ~ geno$bed[snp,] + t(covarBasal[[1]]))
#offset <- colSums(fit$coefficients[-(1:2)]*covarBasal[[1]])
#plotData <- data.frame(genotype=geno$bed[snp,], expression=basalExpr[probe, ]-offset)
#ggplot(plotData, aes(x=factor(genotype), y=expression)) +
#    geom_point(position=position_jitter(width=0.1), size=2.5) +
#    geom_boxplot(alpha=0.4, outlier.colour="white", fill="grey") +
#    theme_bw() + xlab(snp) + ylab(paste(gene, "(log probe intensity)"))
```


Noting that the minor allele frequency of rs12207548 varies considerably between populations we thought to investigate
wether there is evidence of selection at this locus. To this end we calculated F~ST~ between the Yoruban and Central
European populations for this SNP and compare it to all SNPs with similar MAF in the Yoruban samples.
...
geno.ceu <- smList(ceu)
geno.ceu <- mapply(function(x,y) {snps <- colnames(y);snps <- snps[snps %in% colnames(x)]; x[,snps]}, geno.ceu, geno.yri, SIMPLIFY=FALSE)
geno.yri <- mapply(function(x,y) {snps <- colnames(y);snps <- snps[snps %in% colnames(x)]; x[,snps]}, geno.yri, geno.ceu, SIMPLIFY=FALSE)
fst <- mapply(function(x,y) Fst(rbind(x, y), rep(c('ceu', 'yri'), c(nrow(x), nrow(y))))$Fst, geno.ceu, geno.yri, SIMPLIFY=FALSE)
fst <- unlist(fst)
names(fst) <- unlist(lapply(geno.yri, colnames))
```

This estimates the F~ST~ for rs12207548 to be `r round(fst['rs12207548'], 2)`. Of the `r sum(!is.na(fst))` SNPs tested
`r round(sum(fst > fst['rs12207548'], na.rm=TRUE)/sum(!is.na(fst)) * 100, 2)`% have a more extreme value.

Investigating the differences in gene expression between cluster 1 and 2 of the second PLS component
we identified a total of `r nrow(filter(clust1_2Table, adj.P.Val < fdrCut))` probes as differentially
expressed at an FDR threshold of `r fdrCut` (see `r tabRef("clusterTopTable")`
for the top 20 results). Of these `r nrow(filter(clust1_2Table, adj.P.Val < fdrCut & logFC > 0))`
are up and `r nrow(filter(clust1_2Table, adj.P.Val < fdrCut & logFC < 0))` down regulated
in cluster 2 compared to expression levels in cluster 1. Almost all of these (`r nrow(filter(clust1_2Table, adj.P.Val < fdrCut & abs(logFC) > fcCut))`) have a fold change of at least `r 2^fcCut`.
`r figRef("clusterVolcano")` shows a smoothed volcano plot highlighting probes of particular interest.

```{r clusterTopTable, cache=FALSE}
set.caption(tabRef("clusterTopTable", "Top 20 differentially expressed probes."))
rownames(clust1_2Table) <- NULL
pander(head(dplyr::select(clust1_2Table, -ArrayAddress, -EntrezReannotated, -GenomicLocation), 20))
```

```{r clusterLimmaPlot, cache=FALSE, fig.cap=figRef("clusterVolcano", "Volcano plot of differential expression results between clusters 1 and 2. Probes with an adjusted p-value below 0.01 and a log fold cange of at least 0.5 are shown as yellow and red dots.")}
volcanoPlot(clust1_2Table, fcCut=fcCut, fdrCut=fdrCut)
```

```{r clusterHeatmap, cache=FALSE, fig.cap=figRef("clusterExprHeat", "Heatmap comparing gene expression for genes differentially expressed between clusters 1 and 2. Expression estimates for each gene were scaled and centred across samples. Blue cells correspond to lower and red cells to higher than average expression.")}
sortedTable <- dplyr::arrange(dplyr::filter(clust1_2Table, adj.P.Val < fdrCut, abs(logFC) > fcCut), desc(logFC))
sideCols <- c(cluster1="purple", cluster2="orange", cluster3="white")
heatmap.2(sig.data.norm[sortedTable$ArrayAddress, plsComp$treatment == "Stim"], trace = "none", Rowv=FALSE, Colv=as.dendrogram(plsClust), dendrogram="column", scale="row", col=rev(brewer.pal(11, "RdBu")), ColSideColors=sideCols[plsComp$cluster][plsComp$treatment == "Stim"], key=FALSE, labRow="", labCol="", xlab="Samples", ylab="Genes")
```
...
To further characterise the differnces in gene expression we carried out a GO enrichment analysis
for genes differentially expressed between clusters 1 and 2. This reveals strong enrichment for several 
GO categories (`r sum(goTabPlsUp[["cluster1 FDR"]] < 0.05)` and `r sum(goTabPlsDown[["cluster2 FDR"]] < 0.05)`
enriched with FDR < 0.05 in cluster 1 and cluster 2 respectively). Genes with increased
expression in cluster 1 (`r tabRef('tablePlsTopGOUp')`) are enriched for biological processes related to DNA replication whereas those
with increased expression in cluster 2 (`r tabRef('tablePlsTopGODown')`) are annotated with processes related to the heat shock response.

```{r tablePlsTopGOUp, chache=FALSE}
...
```{r tablePlsTopGODown, chache=FALSE}
...
```{r goPlsPlot, cache=FALSE, fig.width=10, fig.height=6, fig.cap=figRef("goPlsPlot", "GO category enrichment for genes differentially expressed between PLS clusters.")}
...
nrows <- max(which(geneP$Gene.FDR <= max(0.05, geneP$Gene.FDR[1])))
nrows <- min(max(nrows, 5), 20)
pander(dplyr::select(geneP[1:nrows,], -NuID))
```
...
highlight <- which(head(geneP_stim)$Symbol %in% stimGenes$Gene)
...
`r if(nrow(stimGenes) > 0 || nrow(basalGenes) > 0) "See" else ""`
`r if(nrow(stimGenes) > 0) tabRef("topGeneSNPs_stim") else ""`
`r if(nrow(stimGenes) > 0 && nrow(basalGenes) > 0) "and" else ""`
`r if(nrow(basalGenes) > 0) tabRef("topGeneSNPs_basal") else ""`
`r if(nrow(stimGenes) > 0 || nrow(basalGenes) > 0) "for a list of SNPs of interest for these genes." else ""`

```{r topGeneSNPs_stim, cache=FALSE, include=nrow(stimGenes) > 0, eval=nrow(stimGenes) > 0}
set.caption(tabRef("topGeneSNPs_stim", paste("Top eSNPs for", stimGenes$Gene[1], "following heat shock.")))
tab <- head(meGene_stim[[stimGenes$NuID[1]]]$all$eqtls, min(geneSNPcount_stim[1], 20))
tab$gene <- stimGenes$Gene[1]
pander(tab)
```

```{r topGeneSNPs_basal, cache=FALSE, include=nrow(basalGenes) > 0, eval=nrow(basalGenes) > 0}
set.caption(tabRef("topGeneSNPs_basal", paste("Top eSNPs for", basalGenes$Symbol[1], "in basal state.")))
tab <- head(meGene_basal[[basalGenes$NuID[1]]]$all$eqtls, min(geneSNPcount_basal[1], 20))
...
stimPlot <- NULL
if(nrow(stimGenes)){
  snp <- as.character(meGene_stim[[stimGenes$NuID[1]]]$all$eqtls$snps[1])
  gene <- stimGenes$Gene[1]
...
basalPlot <- NULL
if(nrow(basalGenes)){
  snp <- as.character(meGene_basal[[basalGenes$NuID[1]]]$all$eqtls$snps[1])
...
known <- unique(dplyr::filter(tab, Symbol %in% knownList$known)$Symbol)
```
...
```{r transGenePlot, cache=FALSE, include=length(known) > 0, eval=length(known) > 0, fig.cap=figRef("transGenesFC", paste("Fold change in gene expression in response to heat shock by genotype for", if(length(known) == 1) known[1] else "known heatshock genes."))}
idx <-sapply(known, function(x) min(which(tab$Symbol == x)))
plots <- vector(mode="list", length(known))
j <- 1
for(i in idx){
  probe <- meHSF$all$eqtls$gene[i]
  snp <- tab$SNP[i]
  transFit <- lm(selectedFC[probe, ] ~ geno$bed[snp,] + t(covar[[1]]))
  transOffset <- colSums(transFit$coefficients[-(1:2)]*covar[[1]])

  plotData <- data.frame(genotype=geno$bed[snp, ], fc=selectedFC[probe, ] - transOffset)
  plotData <- dplyr::filter(plotData, !is.na(genotype))
  plots[[j]] <- ggplot(plotData, aes(x=factor(genotype), y=fc)) +
    geom_point(position=position_jitter(width=0.1), size=2.5) +
    geom_boxplot(alpha=0.4, outlier.colour="white", outlier.size=0, fill="grey") +
    theme_bw() + xlab(tab$SNP[i]) + ylab(paste(tab$Symbol[i], "log fold change"))
  j <- j+1
}
if(length(plots) == 1){
  plots[[1]]
} else{
  cols <- if(length(plots) > 4) 3 else 2
...
```{r transGenePlot2, cache=FALSE, fig.cap=figRef("transGenesTop", paste("Genes with most evidence for genetic modulation of heat shock response based on log fold change and gene expression following stimulation."))}
plots <- vector(mode="list", 2)

probe <- as.character(meHSF$all$eqtls$gene[1])
gene <- dplyr::filter(limmaTable, NuID == probe)$SymbolReannotated[1]
snp <- as.character(meHSF$all$eqtls$snps[1])
transFit <- lm(selectedFC[probe, ] ~ geno$bed[snp,] + t(covar[[1]]))
transOffset <- colSums(transFit$coefficients[-(1:2)]*covar[[1]])

plotData <- data.frame(genotype=geno$bed[snp, ], fc=selectedFC[probe, ] - transOffset)
plotData <- dplyr::filter(plotData, !is.na(genotype))
plots[[1]] <- ggplot(plotData, aes(x=factor(genotype), y=fc)) +
  geom_point(position=position_jitter(width=0.1), size=2.5) +
  geom_boxplot(alpha=0.4, outlier.colour="white", outlier.size=0, fill="grey") +
  theme_bw() + xlab(snp) + ylab(paste(gene, "log fold change"))

probe <- as.character(meHSFstim$all$eqtls$gene[1])
gene <- dplyr::filter(limmaTable, NuID == probe)$SymbolReannotated[1]
snp <- as.character(meHSFstim$all$eqtls$snps[1])
transFit <- lm(stimExpr[probe, ] ~ geno$bed[snp,] + t(covarHSFstim[[1]]))
transOffset <- colSums(transFit$coefficients[-(1:2)]*covarHSFstim[[1]])

plotData <- data.frame(genotype=geno$bed[snp, ], expression=stimExpr[probe, ] - transOffset)
plotData <- data.frame(genotype=geno$bed[snp, ], expression=stimExpr[probe, ] - transOffset)
plotData <- dplyr::filter(plotData, !is.na(genotype))
plots[[2]] <- ggplot(plotData, aes(x=factor(genotype), y=expression)) +
  geom_point(position=position_jitter(width=0.1), size=2.5) +
  geom_boxplot(alpha=0.4, outlier.colour="white", outlier.size=0, fill="grey") +
  theme_bw() + xlab(snp) + ylab(paste(gene, "expression"))
...
```{r, ref.label="functions", eval=FALSE, echo=output=="html"}
```
