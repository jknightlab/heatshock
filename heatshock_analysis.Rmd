...
`r length(unique(dplyr::filter(ggOutlier$data, outlier)[["Cell.Line"]]))` 
obvious outliers, which were removed. In addition the few remaining samples from chip
`r sampleCount <- table(ggOutlier$data$Sentrix_ID); unique(dplyr::filter(ggOutlier$data, Sentrix_ID %in% names(sampleCount)[sampleCount == 4])$Sentrix_ID)` 
