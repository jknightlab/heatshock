...
plsPlot <- function(plsComp, genotype){
  ...
  if(!missing(genotype)){
    ...
    genoAvg <- data.frame(do.call(rbind, by(dplyr::select(plsComp, t1, t2), 
                                            list(plsComp[[genotype]], plsComp$treatment), colMeans)))
    genoAvg[[genotype]] <- rep(levels(plsComp[[genotype]]), length(levels(plsComp$treatment)))
    genoAvg$treatment <- rep(levels(plsComp$treatment), each=length(levels(plsComp[[genotype]])))
    genoAvg$sample <- "genotype"
  }
  
  fig <- ggplot(plsComp, aes(x=t1, y=t2, group=sample))
  if(missing(genotype)){
    fig <- fig + geom_point(data=treatAvg, aes(colour=treatment), size=5)
  } else {
      fig <- fig + geom_point(data=treatAvg, size=5, colour="black")
      fig <- fig + geom_point(data=genoAvg, aes_string(colour=genotype, group=genotype), size=5)
      fig <- fig + geom_line(data=genoAvg, aes_string(colour=genotype, group=genotype),  line.width=2,
  ...
  fig <- fig + geom_line(data=treatAvg, colour="black",  line.width=2,
              arrow=grid::arrow(type="closed", length=grid::unit(0.15, "inches"), angle=20))
  if(missing(genotype)){
    fig <- fig + geom_point(aes(colour=treatment), size=3, alpha=0.4) + 
      geom_point(aes(colour=treatment), size=3, shape=1)
    fig <- fig + scale_colour_manual(values=c(Basal="blue", Stim="red"))
  } else {
    fig <- fig + geom_point(aes_string(colour=genotype), size=3, alpha=0.4) + 
      geom_point(aes_string(colour=genotype), size=3, shape=1)
...
globalPheno <- data.frame(basal.x=basalComp$t1,
                          basal.y=basalComp$t1,
...
two PLS omponents account for `r sum(plsFit$R2[1:2]*100)`% in the variation observed due to 
...
genotypes <- data.frame(sample=rownames(geno$bed), stringsAsFactors=FALSE)
genotypes <- cbind(genotypes, geno$bed[, dimnames(assoc$Results)[[1]][1:10]])
plsComp <- dplyr::left_join(plsComp, genotypes, by="sample")

plsPlot(plsComp, "rs7784431")
```
