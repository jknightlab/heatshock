...
```{r compareGenes}
basal_extended <- dplyr::filter(meBasal, FDR < 0.5)$gene
stim_extended <- dplyr::filter(meStim, FDR < 0.5)$gene

stimGenes <- dplyr::filter(meStim, FDR < 0.05 & !gene %in% basal_extended)
basalGenes <- dplyr::filter(meBasal, FDR < 0.05 & !gene %in% stim_extended)
```
