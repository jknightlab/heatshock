...
goTabPlsUp <- GenTable(tg_pls$up, up = resultPlsUp, down = resultPlsDown, 
                     topNodes = length(score(resultPlsUp)), orderBy = "up")
goTabPlsDown <- GenTable(tg_pls$down, down = resultPlsDown, up = resultPlsUp, 
                     topNodes = length(score(resultPlsDown)), orderBy = "down")
```
...
tblUp <- GenTable(tg$up, up=resultUp, topNodes=length(score(resultUp)))
tblUp$up <- as.numeric(tblUp$up)
tblDown <- GenTable(tg$down, down=resultDown, topNodes=length(score(resultDown)))
...
pander(GenTable(tg_pls$up, up=resultPlsUp, down=resultPlsDown, topNodes=20, orderBy="up"))
```
...
pander(GenTable(tg_pls$down, down=resultPlsDown, up=resultPlsUp, topNodes=20, orderBy="down"))
```
...
tblUp <- GenTable(tg_pls$up, up=resultPlsUp, topNodes=length(score(resultPlsUp)))
tblUp$up <- as.numeric(tblUp$up)
tblDown <- GenTable(tg_pls$down, down=resultPlsDown, topNodes=length(score(resultPlsDown)))
tblDown$down <- as.numeric(tblDown$down)
tbl <- bind_rows(gather(tblUp, "direction", "p.value", up), gather(tblDown, "direction", "p.value", down))
ggplot(tbl, aes(x=Expected, y=Significant)) + 
    geom_point(data=dplyr::filter(tbl, p.value >= 1e-4), colour="grey") +
