...
Further analysis of the gene expression response to heat shock revealed that two of the samples
(NA18502 and NA18508) exhibit patterns of expression inconsistent with those observed
...
```{r extraExclusions}
exclude$sample <- union(exclude$sample, c("NA18502", "NA18508"))
dat <- filterSample(sig.data.long, sig.data.raw.long, det.data.long, exclude$sample)
sig.data.long <- dat$sig
sig.data.raw.long <- dat$sig.raw
det.data.long <- dat$det
...
geneSNPs <- do.call(GRangesList, mapply(
  function(x, chr, s, e) 
    subset(x, seqnames == chr & start >= s - 1e6 & end <= e + 1e6), 
...
idx <- mapply(function(x, chr, s, e, w) which(x$chrom == chr & x$loc >= s - w & x$loc <= e), 
              chr=genePos$chrom, s=genePos$start, e=genePos$end, MoreArgs=list(x=geno$bim, w=window),
              SIMPLIFY=FALSE)
...
pander(head(meGene[[1]]$cis$eqtls, geneSNPcount[1]))
```
...
pander(dplyr::select(head(geneP_stim), -ArrayAddress, -NuID), 
       emphasize.strong.rows=which(head(geneP_stim)$Symbol %in% stimGenes$Symbol))
```
...
pander(dplyr::select(head(geneP_basal), -ArrayAddress, -NuID),
       emphasize.strong.rows=which(head(geneP_basal)$Symbol %in% basalGenes$Symbol))
```
...
in the basal state (`r tabRef("eQTLstimGenes")` and `r tabRef("eQTLbasalGenes")`). See `r tabRef("topGeneSNPs_stim")`
and `r tabRef("topGeneSNPs_basal")` for a list of SNPs of interest for these top genes.

```{r topGeneSNPs_stim, cache=FALSE}
set.caption(tabRef("topGeneSNPs_stim", paste("Possible eQTL for", stimGenes$Symbol[1])))
tab <- head(meGene_stim[[stimGenes$NuID[1]]]$cis$eqtls, geneSNPcount_stim[1])
tab$gene <- stimGenes$Symbol[1]
pander(tab)
```

```{r topGeneSNPs_basal, cache=FALSE}
set.caption(tabRef("topGeneSNPs_basal", paste("Possible eQTL for", basalGenes$Symbol[1])))
tab <- head(meGene_basal[[basalGenes$NuID[1]]]$cis$eqtls, geneSNPcount_basal[1])
tab$gene <- basalGenes$Symbol[1]
pander(tab)
```


```{r topEgeneSNPplot, cache=FALSE, fig.cap=figRef("topEgeneSNP", paste("Gene expression by genotype for genes showing evidence of condition specific eQTL. Gene expression estimates were corrected for the effect of covariates."))}
