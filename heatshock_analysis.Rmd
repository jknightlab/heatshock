...
for(i in idx){
  ...
  snp <- tab$SNP[i]
  transFit <- lm(selectedFC[probe, ] ~ geno$bed[snp,] + t(covarHSF[[1]]))
  transOffset <- colSums(transFit$coefficients[-(1:2)]*covarHSF[[1]])
...
probe <- as.character(meHSF$all$eqtls$gene[i])
...
plots[[1]] <- ggplot(plotData, aes(x=factor(genotype), y=fc)) + 
  geom_point(position=position_jitter(width=0.1), size=2.5) +
  geom_boxplot(alpha=0.4, outlier.colour="white", outlier.size=0, fill="grey") +
  theme_bw() + xlab(snp) + ylab(paste(probe, "log fold change"))

probe <- as.character(meHSFstim$all$eqtls$gene[i])
snp <- as.character(meHSFstim$all$eqtls$snps[1])
transFit <- lm(selectedExpr[probe, ] ~ geno$bed[snp,] + t(covarHSFstim[[1]]))
transOffset <- colSums(transFit$coefficients[-(1:2)]*covarHSFstim[[1]])
  
plotData <- data.frame(genotype=geno$bed[snp, ], expression=selectedExpr[probe, ] - transOffset)
plotData <- dplyr::filter(plotData, !is.na(genotype))
plots[[1]] <- ggplot(plotData, aes(x=factor(genotype), y=expression)) + 
  geom_point(position=position_jitter(width=0.1), size=2.5) +
  geom_boxplot(alpha=0.4, outlier.colour="white", outlier.size=0, fill="grey") +
  theme_bw() + xlab(snp) + ylab(paste(probe, "expression"))
