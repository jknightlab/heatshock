...
There are `r length(unique(dplyr::filter(r_promoter, HSFpeak == 1, novel == 2)$SymbolReannotated))` up-regulated
genes without an established role and
`r length(unique(dplyr::filter(r_promoter, HSFpeak == 1, novel == 1)$SymbolReannotated))` genes with limited
...
pander(dplyr::select(goTabPlsUp, -cluster1, -cluster2))
```
...
pander(dplyr::select(goTabPlsDown, -cluster1, -cluster2))
```
