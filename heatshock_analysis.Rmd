...
countBlocks <- function(geno, pval, pcut=0.05, rcut=0.8){
  ...
  if(length(selected)){
    if(length(selected) == 1){
      ...
      r2 <-cor(geno[, names(pval)[selected]], use="pairwise", method="spearman")^2
      remain <- 1:ncol(r2)
      while(length(remain)){
        i <- remain[1]
        count <- count + 1
...
Significance of the observed associations was assessed through a permutation test.

```{r multiPhenoPerm}
set.seed(54864548L)
nPerm <- 200
pPerm <- numeric(nPerm)
snpPerm <- character(nPerm)
for(i in 1:nPerm){
  permPheno <- dplyr::sample_n(globalPheno, nrow(globalPheno), replace=FALSE)
  assocPerm <- mPhen(cisGeno, permPheno, covariates="sex")
  idx <- which.min(assocPerm$Results[1, ,"JointModel", 2])
  pPerm[i] <- assocPerm$Results[1, idx,"JointModel", 2]
  snpPerm[i] <- dimnames(assocPerm$Results)[[2]][idx]
}
permResult <- data.frame(p.value=pPerm, snp=snpPerm)
```
