...
To assess the magnitude of the global difference in gene expression induced by heat shock
we initially consider the principle components of the gene expression data. We reasoned that
a substantial, global change in gene expression is likely to be well represented by the first
few principle components. To determine which of these best capture the change we carried out
a logistic regression with the treatment state (basal or stimulated) as response that included
the first four principle components as predictors.

```{r pcaFit}
pcaFit <- glm(factor(Treatment) ~ PC1 + PC2 + PC3 + PC4, data=sig.pc, family = binomial())
selPC <- names(sort(coef(pcaFit)[-1], decreasing=TRUE))[1:2]
```
...
```{r probeLoc}
probePos <- as.data.frame(str_split_fixed(selectedGenes$GenomicLocation, ":", 3), stringsAsFactors=FALSE)
names(probePos) <- c("chrom", "start", "end")
probePos <- cbind(id=selectedGenes$NuID, probePos, stringsAsFactors=FALSE)
probePos$start <- as.integer(probePos$start)
probePos$end <- as.integer(probePos$end)
```
...
```{r geneSNPs}
probePos <- makeGRangesFromDataFrame(probePos, ignore.strand = TRUE, keep.extra.columns = TRUE)
snpPos <- makeGRangesFromDataFrame(geno$bim, ignore.strand = TRUE, keep.extra.columns = TRUE, 
                                   start.field = "loc", end.field = "loc")
geneSNPs <- do.call(GRangesList, mapply(
  function(x, chr, s, e) 
    subset(x, seqnames == chr & start >= s - cisWindow & end <= e + cisWindow), 
  as.character(seqnames(probePos)), start(probePos), end(probePos), 
  MoreArgs=list(x=snpPos), SIMPLIFY=FALSE))
names(geneSNPs) <- probePos$id
noSNP <- probePos$id[sapply(geneSNPs, function(x) length(x) == 0)]
...
for(i in 1:nrow(selGenes)){
  ...
  meGene[[selGenes$NuID[i]]] <- Matrix_eQTL_main(
    ...
    errorCovariance = numeric(),
    snpspos = geno$bim,
    genepos = dplyr::select(as.data.frame(probePos), id, seqnames, start, end),
    cisDist = cisWindow,
    verbose = FALSE,
    pvalue.hist = FALSE,
    min.pv.by.genesnp = TRUE,
...
```{r pcaModelTable, cache=FALSE}
set.caption(tabRef("pcaFit", "Logistic regression fit"))
pander(pcaFit)
```

Based on the logistic regression fit (`r tabRef("pcaFit")`) `r paste(selPC, collapse=" and ")` 
were selected as subspace in which to visualise the data (`r figRef("treatPCA")`). While this
shows an overall consitent change in gene expression across samples it also highlights
variability beween individuals.

```{r treatPCAplot, cache=FALSE, fig.cap=figRef("treatPCA", "Samples separate clearly by treatment, showing a consistent global effect on gene expression from heat shock.")}
treatAvg <- data.frame(do.call(rbind, by(dplyr::select(sig.pc, PC2, PC4), sig.pc$Treat, colMeans)))
treatAvg$Treatment <- rownames(treatAvg)
treatAvg[["Cell Line"]] <- "avg"
perp <- rev(treatAvg[2,1:2] - treatAvg[1, 1:2]) * c(-1,1)
slope <- (perp[2]/perp[1])[[1]]
ggplot(sig.pc, aes(x=PC2, y=PC4, colour=Treatment, group=`Cell Line`)) + 
  ...
  geom_abline(slope=slope, intercept=0, colour="black", linetype="dashed") + 
