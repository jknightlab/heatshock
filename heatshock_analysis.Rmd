...
for(i in 1:nrow(selGenes)){
  ...
  snps$fileSkipColumns <- 1
  snps$fileSliceSize <- 2000
  snps$LoadFile(paste0("tmp/ genotypes_", selGenes$Symbol[i],".tab"))
...
write.table(t(pc_stim)[1:nPC_expr, ], file = "tmp/covar_expr_stim.tab", sep="\t", quote=FALSE)
write.table(t(pc_basal)[1:nPC_expr, ], file = "tmp/covar_expr_basal.tab", sep="\t", quote=FALSE)
```
...
```{r runEQTL, cache=FALSE, result="hide"}
...
geneP_basal <- meStim$cis$min.pv.gene * sapply(names(meStim$cis$min.pv.gene), 
                                     function(x) length(geneSNPs[[x]]))
