...
meanCut <- quantile(probeMean, 0.75)
sdCut <- quantile(probeSD[probeMean > meanCut], 0.9)
sig.select <- dplyr::filter(as.data.frame(sig.data.norm), probeSD > sdCut & probeMean > meanCut)
...
```{r vsnPCAcluster, dependson=c("vsnPCA", "vsnOutliers")}
sig.pc$cluster <- 3
...
```{r exprClust}
sig.select <- sig.select[, !grepl(paste0("^", exclude$sample, "_"), colnames(sig.select))]
exprDist <- dist(t(sig.select))
...
dendroData$segments <- merge(dendroData$segments, 
		info[info$Treatment == "Basal",c("Cell Line", "Sentrix_ID", "Operator initial cRNA")], 
		by.x="label", by.y="Cell Line", all.x=TRUE)
dendroData$segments <- merge(dendroData$segments, 
		sig.pc[, c("Cell Line", "cluster")],
		by.x="label", by.y="Cell Line")
dendroData$segments$cluster[is.na(dendroData$segments$cluster)] <- "internal"
dendroData$segments[is.na(dendroData$segments$cluster), "cluster"] <- "internal"
