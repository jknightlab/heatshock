...
snpRange <- sapply(geneSNPs, function(x) paste(x$snp[1], x$snp[length(x)], sep="-"))
snpRange <- paste(snpRange, collapse=",")
writeLines(snpRange, file="tmp/cisRanges", sep="")
```

```{r updatePheno, cache=FALSE}
stimComp <- dplyr::filter(plsComp, treatent=="Stim")
basalComp <- dplyr::filter(plsComp, treatent=="Basal")
...
fam <- as.data.frame(geno$fam[,-ncol(geno$fam)])
names(fam) <- c("family", "individual", "father", "mother", "sex")
fam <- dplyr::left_join(fam, globalPheno, by=c(individual="sample"))
write.table(fam, file="tmp/hapmap_yri.smpl_flt2.fam", row.names=FALSE, col.names=FALSE, 
            quote=FALSE, sep=" ")
```

```{r plinkSubsetSNPs, engine='bash', cache=FALSE}
plink --bfile tmp/hapmap_yri.smpl_flt2 --out tmp/hapmap_yri.cis --snps $(cat tmp/cisRanges) --recode oxford gen-gz
```
