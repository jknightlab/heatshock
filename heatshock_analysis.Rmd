...
shock or, more generally, stress response. As a first step in highlighting genes 
not previously known to play a role in this contxt we identify all significantly
...
```{r goTerms}
sig.entrez <- dplyr::filter(limmaTable, adj.P.Val < fdrCut & logFC > fcCut)$EntrezReannotated
...
knownGenes <- dplyr::filter(gene.data, str_detect(name_1006, paste(termFilter, collapse="|")))
selGenes <- dplyr::filter(limmaTable, adj.P.Val < fdrCut & logFC > fcCut & 
                            !SymbolReannotated %in% knownGenes$hgnc_symbol)
```
...
query <- lapply(queryString, EUtilsSummary)
...
To further establish links between differntially expressed genes and heat shock response
...
for(g in unique(peaks2genes$cond2) ){
        ...
        r <- rbind(r, data.frame(cond=g, up=rst_up$p.value, down=rst_down$p.value, 
                                 sigGenes_up=groupUp_in_anno_genes, sigGenes_down=groupDown_in_anno_genes) )
}

heatshock_novel_genes <- unique(putativeGenes$SymbolReannotated)
genes_in_both$heatshock_novel_genes <- ifelse( genes_in_both$genes %in% heatshock_novel_genes, 1, 0)
```

```{r peakOutput, cache=FALSE}
write.table(r, "tmp/hsf_genes_pvalue.txt", sep="\t", quote=FALSE, row.names=FALSE)
write.table(genes_in_both, "tmp/hsf_genes.txt" , sep="\t", quote=FALSE, row.names=FALSE)
system("tar -czf tmp/gene_annotation.tar.gz -C tmp heatshock_pmid.txt heatshock_novel.tab hsf_genes.txt hsf_genes_pvalue.txt")
```
...
the joint model were computed for each permutation. From these we computed fase discovery rates
...
```{r heatmap, cahe=FALSE, fig.cap=figRef("exprHeat", "Heatmap comparing gene expression for differentially expressed genes between basal and stimulated samples. Samples were clustered by gene  with stimulated (orange) and basal (purple) samples forming two distinct groups. Expression estimates for each gene were scaled and centred across samples. Blue cells correspond to lower and red cells to higher than average expression.")}
...
Of the `r length(unique(sig.entrez))` significantly up regulated genes `r length(unique(knownGenes$entrezgene))`
are associated with GO terms that clearly relate to heat shock response or other relevant processes
and a further `r length(pmids)` genes with at least one reference to heat shock or stress response.
This leaves `r length(queryEmpty)` genes with no obvious association to heat shock. Of these 
`r sum(is.na(putativeGenes$go_id))` don't have any GO annotations.

## Global heat shock response

Considering the outcome of the partial least squares regression it is aparent that there is
