...
## Create link to AMIGO page for GO term
linkGO <- function(term){
  paste0('<a href="http://amigo.geneontology.org/amigo/term/', term, '">', term, '</a>')
}
...
tbl <- GenTable(tgUp, up=resultUp, down=resultDown, topNodes=20, orderBy="up")
tbl[["GO.ID"]] <- linkGO(tbl[["GO.ID"]])
set.caption(tabRef("topGOUp", "Top 20 GO categories enriched for up regulated genes."))
pander(tbl)
```

```{r tableTopGODown, chache=FALSE}
tbl <- GenTable(tgDown, down=resultDown, up=resultUp, topNodes=20, orderBy="down")
tbl[["GO.ID"]] <- linkGO(tbl[["GO.ID"]])
