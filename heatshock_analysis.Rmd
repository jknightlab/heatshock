...
exclude$samples <- union(exclude$samples, ggOutlier$data["outlier", "Cell Line"])
ggOutlier
```
...
clustering by BeadChip, as well as `r length(unique(ggOutlier$data["outlier", "Cell Line"])) - 1` 
obvious outliers, which were removed. In addition the only remaining sample from chip
563403729 was removed as the entire chip appears to be suspect.
...
batch <- with(subset(info, !`Cell Line` %in% exclude$sample), 
		data.frame(sample=sample, cluster=as.integer(factor(Sentrix_ID)), 
			treatment=info$Treatment, stringsAsFactors=FALSE))
