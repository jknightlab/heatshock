...
outlierPlot <- function(data, x="PC1", y="PC2", group="Sentrix_ID", 
			shape="Treatment", level=0.95, ...){
	ellipses <- by(data[, c(x, y)], data[[group]], 
			function(d) tryCatch(dataEllipse(as.matrix(d), levels=level, draw=FALSE, robust=TRUE), 
	...
	ggplot(data, aes_string(x=x, y=y, colour=group)) +
			geom_point(aes_string(shape=shape, fill="outlier"), size=3) + 
			stat_ellipse(level=level) + facet_wrap(as.formula(paste("~", group))) +
			theme_bw() + scale_colour_discrete(guide="none") +
            scale_shape_manual(values=c(Basal=21, Stim=24)) +
            scale_fill_manual(values=c("TRUE"="grey", "FALSE"="white"), guide="none")
}
...
```{r vsnOutliers, debendson="vsnPCA"}
...
```{r vsnPCAplot, cache=FALSE, fig.cap=figRef("vsnPCA", "PCA plot of normalised gene expression estimates by sample. Three clusters are clearly visible. Cluster 1 is composed of samples from a single BeadChip (ID 536403752). The remaining two clusters show some correlation with RNA extraction day but the correspondence is not perfect.")}
ggplot(sig.pc, aes(x=PC1, y=PC2)) +  
	geom_point(aes( colour=as.character(cluster), shape=Treatment), size=3) + 
     theme_bw() + scale_colour_discrete("Cluster")
```

```{r excludeOutliers, ref.label="filterSample", dependson="vsnOutliers"}
```

```{r exclOutlGeno, ref.label="sampleList", cache=FALSE}
```

```{r vsn2, ref.label="vsn", dependson="excludeOutliers"}
```

```{r selectProbes2, ref.label="selectProbes", dependson="vsn2"}
```
...
```{r exprClust}
exprDist <- dist(t(dplyr::select(sig.select, -mean, -sd)))
...
dendroData$segments <- left_join(dendroData$segments, dendroData$labels, by=c(x="x", yend="y"))

dendroData$segments <- left_join(dendroData$segments, dplyr::select(sig.pc, -(sample:PC4)),
		by=c(label="Cell Line", treatment="Treatment"))
...
ggplot(dendroData$segments) + geom_segment(aes(x=x, y=y, xend=xend, yend=yend, linetype=treatment, colour=cluster)) + 
 ...
 #       scale_colour_manual(values=c("1"="orange", "2"="red", "3"="grey", "internal"="black")) +
