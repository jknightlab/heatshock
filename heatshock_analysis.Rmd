...
Of the `r length(unique(sig.entrez))` significantly up regulated genes 
`r length(unique(dplyr::filter(limmaTable, SymbolReannotated %in% knownList$known, logFC > fcCut, adj.P.Val < fdrCut)£SymbolReannotated` have been directly linked to the heat shock response and 
`r length(setdiff(knownList$extended))))`
