...
for(i in idx){
  plotData <- data.frame(genotype=geno$bed[tab$SNP[i], ], fc=selectedFC[meHSF$all$eqtls$snps[i]])
  plotData <- dplyr::filter(plotData, !is.na(genotype))
  ggplot(plotData, aes(x=genotype, y=fc)) + 
    geom_points(position=position_jitter(width=0.1), size=2.5) +
