...
filterSample <- function(sig, sig.raw, det, excl){
  ...
  return(sig=sig, sig.raw=sig.raw, det=det)
}
...
```{r filterSample}
dat <- filterSample(sig.data.long, sig.data.raw.long, exclude$sample)
...
```{r extraExclusions}
exclude$sample <- union(exclude$sample, c("NA18502", "NA18505"))
dat <- filterSample(sig.data.long, sig.data.raw.long, exclude$sample)
sig.data.long <- dat$sig
sig.data.raw.long <- dat$sig.raw
det.data.long <- dat$det
...
updateSampleList(sig.data.long$sample, "data/hapmap_YRI_r23a_filtered.fam")
```
...
```{r filterSample2}
dat <- filterSample(sig.data.long, sig.data.raw.long, eclude$sample)
sig.data.long <- dat$sig
sig.data.raw.long <- dat$sig.raw
det.data.long <- dat$det
```
