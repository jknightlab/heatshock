...
```{r writeSNPpos, cache=FALSE, echo=FALSE}
...
```{r writeGeno, cache=FALSE, echo=FALSE}
...
```{r prepQTL, echo=FALSE, cache=FALSE}
snps <- SlicedData$new();
snps$fileDelimiter <- "\t"
...
mainSNP <- dplyr::filter(me$cis$eqtl, !duplicated(gene))
...
for(i in 1:nrow(selGenes)){
  selSNPs <- geno$bed[geneSNPs[[selGenes$NuID[i]]], ]
  write.table(selSNPs, file=paste0("tmp/ genotypes_", selGenese$Symbol[i],".tab"), 
              quote=FALSE, sep="\t")
  write.table(selectedFC[selGenes$NuID[i], , drop=FALSE], 
              file=paste0("tmp/", selGenese$Symbol[i], "_fc.tab"), sep="\t", quote=FALSE)
}
```

```{r runGeneMatrixEqtl, cache=FALSE}
...
for(i in 1:nrow(selGenes)){
  ...
  snps$fileSkipColumns <- 1
  snps$fileSliceSize <- 2000
  snps$LoadFile(paste0("tmp/ genotypes_", selGenese$Symbol[i],".tab"))
  ...
  pheno$fileSkipColumns <- 1
  pheno$fileSliceSize <- 2000
  pheno$LoadFile("tmp/", selGenese$Symbol[i], "_fc.tab")
  
  meGene[[selGenes$NuID[i]]] <- Matrix_eQTL_main(
    snps = snps,
    gene = pheno,
    cvrt = covar,
    output_file_name.cis = paste0("tmp/reQTL_", selGenese$Symbol[i],"analysis"),
    pvOutputThreshold = 0,
