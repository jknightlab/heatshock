...
```{r compareGenes}
basal_extended <- dplyr::filter(meBasal$all$eqtls, FDR < 0.5)$gene
stim_extended <- dplyr::filter(meStim$all$eqtls, FDR < 0.5)$gene

stimGenes <- dplyr::filter(meStim$all$eqtls, FDR < 0.05 & !gene %in% basal_extended)
basalGenes <- dplyr::filter(meBasal$all$eqtls, FDR < 0.05 & !gene %in% stim_extended)
```
