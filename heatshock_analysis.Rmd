...
plink --bfile data/hapmap_YRI_r23a_filtered --out hapmap_yri.snp_flt --keep data/sample.lst --maf 0.1 --geno 0.1 --hwe 1e-4 --split-x b36 no-fail --autosome-xy --make-bed --silent
```

```{r snpFilter}
totalSNPs <- R.utils::countLines("data/hapmap_YRI_r23a_filtered.bim")
remainSNPs <- R.utils::countLines("hapmap_yri.bim")
```
...
plink --bfile  hapmap_yri.snp_flt --out hapmap_yri.smpl_flt --mind 0.1 --neighbour 1 3 --make-bed
```
