...
clusterChip2 <- dplyr::filter(sig.pc, Sentrix_ID %in% c(526786722, 526786736, 
				526786743, 526786749, 536403760, 536403729, 536403730) & !outlier)
...
ggplot(sig.pc, aes(x=PC1, y=PC2)) +  
	 geom_polygon(data=clusterOp[clusterOpBound_1_2, ], fill="lightgrey", colour="grey") +
	 geom_polygon(data=clusterChip1[clusterChip1Bound_1_2, ], alpha=0.8, fill="yellow", colour="orange") +
	 geom_polygon(data=clusterChip2[clusterChip2Bound_1_2, ], alpha=0.8, fill="lightblue", colour="blue") +
...
dendroData$segments <- mutate(dendroData$segments, 
		cluster=ifelse(Sentrix_ID %in% c(526786720, 526786721), "1", 
		ifelse(Sentrix_ID == 536403752, "2", "3")))
...
ggplot(dendroData$segments) + geom_segment(aes(x=x, y=y, xend=xend, yend=yend, linetype=treatment, colour=cluster)) + 
        ...
        scale_colour_manual(values=c("1"="grey", "2"="orange", "3"="blue", "internal"="black")) +
...
```{r vsnPCAcluster2}
clusterChip3<- dplyr::filter(sig.pc, Sentrix_ID %in% c(526786720, 526786721))
clusterChip3Bound_1_2 <- chull(clusterChip3$PC1, clusterChip4$PC2) 
clusterChip3Bound_1_3 <- chull(clusterChip3$PC1, clusterChip4$PC3) 
...
clusterChip2 <- dplyr::filter(sig.pc, Sentrix_ID %in% c(526786722, 526786736, 
				526786743, 526786749, 536403760, 536403729, 536403730))
clusterChip2Bound_1_2 <- chull(clusterChip2$PC1, clusterChip3$PC2)
clusterChip2Bound_1_3 <- chull(clusterChip2$PC1, clusterChip3$PC3)
```
...
ggplot(sig.pc, aes(x=PC1, y=PC2)) +  
	 geom_polygon(data=clusterChip1[clusterChip1Bound_1_2, ], 
	 		alpha=0.8, fill="yellow", colour="orange") +
	 geom_polygon(data=clusterChip2[clusterChip2Bound_1_2, ], 
	 		alpha=0.8, fill="lightblue", colour="blue") +
	 geom_polygon(data=clusterChip3[clusterChip3Bound_1_2, ], 
	 		alpha=0.8, fill="lightgrey", colour="grey") +
...
dendroData$labels$label <- sapply(labels, "[[", 1)
dendroData$labels$treatment <- sapply(labels, "[[", 2)
dendroData$segments <- merge(dendroData$segments, dendroData$labels, 
		by.x=c("xend", "yend"), by.y=c("x", "y"), all.x=TRUE)
dendroData$segments[is.na(dendroData$segments$treatment), "treatment"] <- "internal"
info[["Cell Line"]] <- sub("_.$", "", info[["Cell Line"]])
...
ggplot(dendroData$segments) + geom_segment(aes(x=x, y=y, xend=xend, yend=yend, 
			linetype=treatment, colour=cluster)) + 
        scale_x_discrete(label=filter(dendroData$segments, label != "internal")$label) + 
        scale_linetype_manual(values=c(Basal="solid", Stim="dashed", internal="solid")) +
        scale_colour_manual(values=c("1"="grey", "2"="orange", "3"="blue", "internal"="black")) +
        theme_dendro() + theme(axis.text.x = element_text(angle=90, hjust=1))
